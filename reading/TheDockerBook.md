...
2016.4.22
#
Docker有一个功能叫连接（link），非常有用，可以把一个或多个Docker容器连接起来，让它们互相通信。

sudo docker run的新标志： --link
创建两个容器间的父子连接。例子：
```
sudo docker run -p 4567 --name webapp --link redis:db -t -i \
-v $PWD/webapp:/opt/webapp fengyao/sinatra /bin/bash
```
--link标志需要两个参数，一个是要连接容器的名字（redis），另一个是连接后容器的别名（db）。
别名让我们可以访问公开的信息，而无需关注底层容器的名字。
连接让父容器有能力访问子容器，并且把子容器的一些连接细节分享给父容器，
这些细节有助于配置应用程序并使用这个连接。
通过把容器连接到一起，可以让父容器直接访问任意子容器的公开端口。
更秒的是：只有使用--link标志连接到这个容器的容器才能连接到这个端口。
容器的端口不需要对宿主机公开，现在我们拥有一个非常安全的模型。
在这个模型中，容器化的应用程序限制了可被攻击的界面，减少了公开暴露的网络。
